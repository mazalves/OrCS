...
class cache_manager_t {
    ...
    private:
        ...
        void add_mshr_entry(memory_package_t* mob_line, uint64_t latency_request, bool hive);
        ...
        uint32_t recursiveDataSearch(memory_package_t *mob_line, uint64_t instructionAddress, int32_t *cache_indexes, uint32_t latency_request, uint32_t ttc, uint32_t cache_level, cacheId_t cache_type);
